...
def main():
    ...
    try:
        for i, g in enumerate(SeqIO.parse(args.genbank, 'genbank')):
            ...
            try:
                ...
                if '_' in record['accession']:
                    refseq = parse_refseq_source(g)
                    refseq.update({'seqname': record['seqname']})
                    args.refseq_info.writerow(refseq)
                    record.update({'original': refseq['accession']})
        ...
        print('previous genbank ({}) ({})'.format(i, g))
        raise e
...
def parse_record(record):
    ...
    info = dict(accession=accession,
                ...
                original=None,  # updated after parse_refseq_source
                seq_start=seq_start,
                seq_stop=seq_stop,
                seqname=seq_id,
